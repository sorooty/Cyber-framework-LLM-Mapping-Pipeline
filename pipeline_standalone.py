...
def _parse_survey_ts(path: Path = SURVEY_TS_PATH) -> dict:
    ...
    if not m:
        raise ValueError(
            "Impossible de localiser defineSurveyConfig({...}) dans survey.ts"
        )
    raw = "{" + m.group(1) + "}"
    raw = re.sub(r"//[^\n]*", "", raw)
    raw = raw.replace("'", '"')
    ...
    try:
        ...
        raise ValueError(
            f"Erreur de parsing JSON survey.ts (ligne {e.lineno}) :\n{context}\n\n{e}"
        )
...
_FRAMEWORK_METADATA: dict[str, dict] = {
    "cis_controls_v8": {
        ...
        "language": "en",
        "section_id_prefix": "CIS-",
    },
    "nistCsfV2": {
        "display_name": "NIST CSF v2",
        "domain": "cybersecurity",
        "language": "en",
    },
    ...
    "qualiopiV9": {
        "display_name": "Qualiopi v9",
        "domain": "training",
        "language": "fr",
    },
    "anssiHygieneGuideV2": {
        "display_name": "ANSSI Guide d'Hygiène v2",
        "domain": "cybersecurity",
        "language": "fr",
    },
    "anssiAISecurity": {
        "display_name": "ANSSI AI Security",
        "domain": "ai",
        "language": "fr",
    },
    "owaspV4_0": {
        "display_name": "OWASP ASVS v4.0",
        "domain": "cybersecurity",
        "language": "en",
    },
    "owasp_llm_ai_v1": {
        "display_name": "OWASP LLM AI v1",
        "domain": "ai",
        "language": "en",
    },
    ...
    "afnor_spec": {
        "display_name": "AFNOR Spec AI 22-17",
        "domain": "ai",
        "language": "fr",
    },
    "secnumcloud_v3": {
        "display_name": "SecNumCloud v3",
        "domain": "cloud",
        "language": "fr",
    },
    "igi11300": {
        "display_name": "IGI 1300:2021",
        "domain": "classified",
        "language": "fr",
    },
    "ii901": {"display_name": "II 901", "domain": "cybersecurity", "language": "fr"},
    "iec_62443_3_2": {
        "display_name": "IEC 62443-3-2:2020",
        "domain": "ot_security",
        "language": "en",
    },
    "ichE6R2": {"display_name": "ICH E6(R2)", "domain": "medical", "language": "en"},
    "isa6_0_3_tisax": {
        "display_name": "ISA6 / TISAX",
        "domain": "automotive",
        "language": "en",
    },
    "ai_act": {"display_name": "EU AI Act", "domain": "ai", "language": "mixed"},
    "fda_cfr_21_part": {
        "display_name": "FDA CFR 21 Part 11",
        "domain": "medical",
        "language": "en",
    },
    "cosmetovigilance": {
        "display_name": "Cosmétovigilance 2009",
        "domain": "cosmetics",
        "language": "fr",
    },
...
def load_generic_yaml(
    path: str, framework_name: str | None = None
...
def load_framework(
    ...
    if validate and framework_name:
        try:
            ...
            pass  # Framework not registered in survey.ts
        except Exception as e:
            print(f"  [WARN] validate_by_yaml_name({framework_name}): {e}")
    return requirements
...
def _validate_ingestion(
    source_stem: str, requirements: list[RequirementNormalized]
) -> None:
    if not requirements:
        return
    ...
    try:
        ...
        if report.ok:
            print(
                f"  [✅ survey.ts] {framework_name} : {report.total_parsed}/{report.total_expected}"
            )
        else:
            print(
                f"  [⚠️  survey.ts] {framework_name} : "
        ...
        pass  # Framework not registered in survey.ts — expected for some frameworks
    except Exception as e:
        print(f"  [WARN] _validate_ingestion({source_stem}): {e}")
...
def run_ingestion(
    ...
    print(
        f"[Étape 1] Ref A : {len(ref_A)} exigences  |  Ref B : {len(ref_B)} exigences"
    )
    return ref_A, ref_B
...
def run_ingestion_all(
    ...
    for path in paths:
        ...
        cache = (
            DATA_DIR / f"{re.sub(r'[^a-z0-9]', '_', fw_key.lower())}_normalized.json"
        )
...
def _get_embedding_model() -> SentenceTransformer:
    if not hasattr(_get_embedding_model, "_model"):
        try:
            ...
            device = "cpu"
        print(
            f"  Chargement du modèle d'embedding ({EMBEDDING_MODEL}) sur {device.upper()}..."
        )
...
def _save_matrix_cache(matrix: np.ndarray, path: Path) -> None:
    """Saves a numpy matrix and a companion .meta.json with the cache version."""
    np.save(str(path), matrix)
    meta = path.with_suffix(".meta.json")
    with open(meta, "w", encoding="utf-8") as f:
        json.dump({"_v": CACHE_VERSION}, f)


def _matrix_cache_valid(path: Path) -> bool:
    """Returns True only if the .npy file and its .meta.json exist with matching version."""
    if not path.exists():
        return False
    meta = path.with_suffix(".meta.json")
    if not meta.exists():
    ...
    try:
        with open(meta, encoding="utf-8") as f:
            return json.load(f).get("_v") == CACHE_VERSION
    except Exception:
        return False
...
def run_similarity(
    ...
    emb_A = model.encode(
        [_req_text(r) for r in ref_A], show_progress_bar=True, convert_to_numpy=True
    )
    emb_B = model.encode(
    ...
    print(
        f"  {len(pairs)} paires candidates retenues (seuil={semantic_threshold}, top_k={top_k})."
    )
    print(
        f"  Reduction : {total} combinaisons -> {len(pairs)} paires ({100 * len(pairs) / total:.1f}%)"
    )
    return pairs, matrix
...
def run_similarity_all(
    ...
    if not force:
        all_cached = all(
            ...
            and _matrix_cache_valid(DATA_DIR / f"similarity_matrix_{_cache_slug(fw_i, fw_j)}.npy")
    ...
    for fw_i, fw_j in pairs_iter:
        ...
        print(
            f"  [{fw_i} <-> {fw_j}] {len(pairs)}/{total_comb} paires "
    ...
    total_pairs = sum(len(v) for v in candidates_all.values())
    print(
        f"[Étape 2] Total : {total_pairs} paires candidates sur {len(pairs_iter)} combinaisons"
    )
    return candidates_all, matrices_all
...
def run_heatmap(
    ...
    ax.set_title(
        "Matrice de similarité sémantique — titre + description", fontsize=14, pad=12
    )
...
async def _run_scorer_async(
    ...
    batches = [
        ...
        for i in range(0, len(pair_objects), batch_size)
    ]
    cand_batches = [
...
def run_scorer(
    ...
    fw_a_name = ref_A[0].framework if ref_A else ""
    fw_b_name = ref_B[0].framework if ref_B else ""
    for r in relations:
        if not r.framework_A:
            r.framework_A = fw_a_name
        if not r.framework_B:
            r.framework_B = fw_b_name
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Pipeline de mapping référentiels (standalone)"
    )
    ...
    parser.add_argument(
        "--ref-a", default=None, help="Chemin vers le fichier Ref A (YAML)"
    )
    parser.add_argument("--adapter-a", default="generic")
    parser.add_argument(
        "--ref-b", default=None, help="Chemin vers le fichier Ref B (YAML)"
    )
    ...
    parser.add_argument(
        ...
        help=f"Seuil de similarité sémantique (défaut: {SEMANTIC_THRESHOLD})",
    )
    parser.add_argument(
        "--top-k",
        type=int,
        default=TOP_K,
        help=f"Nombre max de candidats par exigence (défaut: {TOP_K})",
    )
    parser.add_argument(
        "--skip-step4", action="store_true", help="Arrêter après l'étape 3"
    )
    parser.add_argument(
        "--skip-step5", action="store_true", help="Ne pas filtrer les aucun_lien"
    )
    parser.add_argument(
        "--min-confidence",
        type=float,
        default=LLM_MIN_CONFIDENCE,
    ...
    if args.all_frameworks:
        yaml_paths = sorted(SURVEYS_DIR.glob("*.yaml")) + sorted(
            SURVEYS_DIR.glob("*.yml")
        )
        if not yaml_paths:
    ...
    if loader_a is None:
        sys.exit(
            f"Adaptateur inconnu : {args.adapter_a!r}. Disponibles : {list(ADAPTER_LOADERS)}"
        )
    if loader_b is None:
        sys.exit(
            f"Adaptateur inconnu : {args.adapter_b!r}. Disponibles : {list(ADAPTER_LOADERS)}"
        )
    ...
    ref_A, ref_B = run_ingestion(
        ...
        path_B=args.ref_b,
        loader_B=loader_b,
        cache_name_A=f"{fw_a}_normalized.json",
        cache_name_B=f"{fw_b}_normalized.json",
